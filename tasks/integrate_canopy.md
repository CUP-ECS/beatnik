...
| 7 | FmmBRSolver: full FMM compute path | c2b5250 | `spack install` succeeds (1m 2s). Pipeline: pack → setup → solve → cross-product → reverse-distribute → write zdot. Runtime equivalence against `-S exact` on a 32x32 free-boundary single-rank case is the final verification step for this checkpoint, deferred until the test framework is agreed on. v1 simplification: `setup()` runs every step; switching to `auto_maintain` is a follow-up. |
