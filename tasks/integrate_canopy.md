...
| 6 | FmmBRSolver: first-call setup + grid pack (no solve, no Distributor) | 227a61e | (as above) |
| 7 | FmmBRSolver: full FMM compute path | _pending_ | `spack install` succeeds (1m 2s). Pipeline: pack → setup → solve → cross-product → reverse-distribute → write zdot. Runtime equivalence against `-S exact` on a 32x32 free-boundary single-rank case is checkpoint 7's final verification step, deferred to the next session. v1 simplification: `setup()` runs every step; switching to `auto_maintain` is a follow-up. | `spack install` succeeds (1m 12s) after installing Canopy first via `spack install canopy@develop … %cce` and fixing Canopy's `CanopyConfig.cmakein` to `find_dependency(Trilinos)` (Canopy commit `ab1e8fc`). Compute path still falls through to ring-pass all-pairs after first-call setup. |
