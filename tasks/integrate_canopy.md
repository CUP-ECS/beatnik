...
| 7a | Add `Beatnik_Test_FmmVsExact` CTest + spack `+testing`/`+canopy` variants | _pending_ | Two TEST cases in `tests/tstFmmVsExact.hpp`: (a) BR-direct comparison with manually-set z and non-zero omega; (b) one-RK3-step smoke test that constructs full Solvers and steps once. spack `package.py` in compass repo gains `+testing` and `+canopy` variants. CLAUDE.md "Minimum test set" updated. Pending user reconcretize + verify canopy installs before beatnik. |
