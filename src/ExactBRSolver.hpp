...
class ExactBRSolver
{
  public:
	...
	Kokkos::MDRangePolicy<ExecutionSpace, Kokkos::Rank<4>> pair_policy({node_space.min(0), node_space.min(1), 
									      node_space.min(0), node_space.min(1)}, 
							                     {node_space.max(0), node_space.max(1), 
									      node_space.max(0), node_space.max(1)});
