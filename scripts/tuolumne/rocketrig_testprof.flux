#!/bin/bash
# flux: --job-name=rocketrig_testprof
# flux: --nodes=4
# flux: --exclusive
# flux: --time=30
...
BASE_DIR="/p/lustre5/stewartj/beatnik/fmm/testprof"
RUN_DIR="${BASE_DIR}/fmm"
SRC_INPUT="/p/lustre5/stewartj/beatnik/fmm/debug/single_mode_debug.in"
...
# with the exact script). Also cap this profiling run at 600 timesteps
# (the base deck runs longer) and disable output by setting
# write_frequency to 0; only those lines are rewritten.
sed -e 's/^br_solver.*/br_solver            = fmm/' \
    -e 's/^timesteps.*/timesteps            = 600/' \
    -e 's/^write_frequency.*/write_frequency      = 0/' "${SRC_INPUT}" \
